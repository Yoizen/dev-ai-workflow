...
function Install-SDD {
    ...
    switch ($Action) {
        "install" {
            Write-Info "Installing SDD Orchestrator..."
        ...
        "update" {
            Write-Info "Updating SDD Orchestrator..."
            # Re-install to get latest skills
            return (Install-SDD -Action "install" -TargetDir $TargetDir)
        }
        
        "skip" {
            Write-Info "Skipping SDD Orchestrator installation"
            return $true
        }
...
function Install-VscodeExtensions {
    ...
    if (-not $codeCmd) {
        Write-Warning "VS Code CLI not available, skipping extensions"
        return $true
    }
    ...
    switch ($Action) {
        ...
        "skip" {
            Write-Info "Skipping VS Code extensions"
            return $true
        }
...
function Install-OpenCodeCli {
    if (Get-Command opencode -ErrorAction SilentlyContinue) {
        Write-Info "OpenCode CLI already installed"
        return $true
    }

    if (-not (Get-Command npm -ErrorAction SilentlyContinue)) {
        Write-Warning "npm not available, skipping OpenCode CLI install"
        return $true
    }
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Success "OpenCode CLI installed"
        return $true
    }

    Write-Warning "Could not install OpenCode CLI"
    return $true
}
...
function Set-ProjectConfiguration {
    ...
    Write-Info "Configuring project at $TargetDir..."
    ...
    if (-not $SkipGa) {
        ...
        if ($gaCmd) {
            Write-Info "Initializing GA in repository..."
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Info "Installing GA hooks..."
                Push-Location $TargetDir
                ...
                if ($LASTEXITCODE -eq 0) {
                    ...
                    Write-Warning "GA hook installation had issues"
                }
            } else {
                Write-Warning "Failed to initialize GA"
            }
        } else {
            Write-Warning "GA command not available, skipping initialization"
        }
    }
    ...
    if ($lefthookCmd) {
        ...
        if (-not (Test-Path $lefthookConfig)) {
            ...
            $lefthookTemplate = if (Test-Path $typeLefthookTemplate) {
                ...
                Join-Path $autoDir "autohook.yml.template"
            }
            if (Test-Path $lefthookTemplate) {
                ...
                Write-Info "Installing Lefthook hooks..."
                Push-Location $TargetDir
                ...
                if ($LASTEXITCODE -eq 0) {
                    ...
                    Write-Warning "Lefthook installation had issues"
                }
            } else {
                Write-Warning "Lefthook template not found"
            }
        } else {
            Write-Info "lefthook.yml already exists"
        }
    } else {
        Write-Info "Lefthook not installed, skipping hook configuration"
    }
    ...
    if (Test-Path $skillsSetupPs) {
        Write-Info "Configuring AI skills (Copilot + OpenCode) via setup.ps1..."
        Push-Location $TargetDir
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Warning "AI skills setup had issues"
        }
    } elseif (Test-Path $skillsSetup) {
        $bashCmd = Get-Command bash -ErrorAction SilentlyContinue
        if ($bashCmd) {
            Write-Info "Configuring AI skills (Copilot + OpenCode) via setup.sh..."
            Push-Location $TargetDir
            ...
            Write-Info "bash not found, skipping skills/setup.sh auto-configuration"
        }
    } else {
        Write-Warning "skills/setup.ps1/setup.sh not found, skipping AI skills setup"
    }
...
function Set-BiomeBaseline {
        ...
        Write-Info "Configuring optional Biome baseline..."

        $biomeConfig = Join-Path $TargetDir "biome.json"
        $packageJson = Join-Path $TargetDir "package.json"

        if (Test-Path $biomeConfig) {
                Write-Info "biome.json already exists, skipping baseline config file"
        } else {
                $biomeContent = @'
{
...
'@
        ...
        if (-not (Test-Path $packageJson)) {
                Write-Warning "package.json not found, skipping Biome package/scripts setup"
                return $true
        }

        $pkgRaw = Get-Content $packageJson -Raw
        if ($pkgRaw -match '"@biomejs/biome"') {
                Write-Info "@biomejs/biome already present in package.json"
        } else {
                $npmCmd = Get-Command npm -ErrorAction SilentlyContinue
                if (-not $npmCmd) {
                        Write-Warning "npm not found, skipping @biomejs/biome automatic install"
                } else {
                        Write-Info "Installing @biomejs/biome..."
                        Push-Location $TargetDir
                        ...
                        if ($LASTEXITCODE -eq 0) {
                                ...
                                Write-Warning "Failed to install @biomejs/biome automatically"
                        }
                }
        }
...
" 2>&1 | Out-Null
                        ...
                        if ($LASTEXITCODE -eq 0) {
                                ...
                                Write-Warning "Failed to update package.json scripts for Biome"
                        }
                } finally {
                        Pop-Location
                }
...
function Install-Extensions {
    ...
    switch ($Action) {
        "install" {
            Write-Info "Installing type extensions..."

            # Determine source root (prefer local repo source)
            $scriptParent = Split-Path -Parent $PSScriptRoot # lib
            $repoRoot = Split-Path -Parent $scriptParent     # setup/.. = root

            # Base install-step: github-prompts
            $localPromptsSource = Join-Path $repoRoot "extensions\install-steps\github-prompts\prompts"
            $installedPromptsSource = Join-Path $GA_DIR "extensions\install-steps\github-prompts\prompts"
            $promptsSource = ""
            if (Test-Path $localPromptsSource) { $promptsSource = $localPromptsSource }
            elseif (Test-Path $installedPromptsSource) { $promptsSource = $installedPromptsSource }

            if ($promptsSource) {
                $promptsTarget = Join-Path $TargetDir ".github\prompts"
                New-Item -ItemType Directory -Path $promptsTarget -Force | Out-Null
                $copiedPrompts = 0
                Get-ChildItem -Path $promptsSource -Filter "*.md" -File -ErrorAction SilentlyContinue | ForEach-Object {
                ...
                Write-Success "Installed $copiedPrompts GitHub prompt file(s)"
            }

            # Base MCP example: context7
            $mcpDir = Join-Path $TargetDir ".ywai\mcp"
            $context7File = Join-Path $mcpDir "context7-mcp.example.json"
            if (-not (Test-Path $context7File)) {
...
'@ | Set-Content -Path $context7File
            ...
            # Base install-step: engram-setup
            $engramDir = Join-Path $TargetDir ".ywai\engram"
            $engramStatus = Join-Path $engramDir "status.txt"
            $engramReadme = Join-Path $engramDir "README.md"
...
If the `engram` CLI is available, setup attempts to configure:
- OpenCode
- Codex
- Gemini CLI
...
'@ | Set-Content -Path $engramReadme

            $engramCmd = Get-Command engram -ErrorAction SilentlyContinue
            if ($engramCmd) {
                $engramVersion = (& engram version 2>$null | Select-Object -First 1)
                $configuredTargets = 0
                $failedTargets = 0
                foreach ($engramTarget in @("opencode", "codex", "gemini-cli")) {
                    & engram setup $engramTarget 2>&1 | Out-Null
                    if ($LASTEXITCODE -eq 0) {
                        $configuredTargets++
                    } else {
                        $failedTargets++
                ...
                @"
engram: installed
version: $($engramVersion | ForEach-Object { $_ } | Select-Object -First 1)
auto_configured: yes
configured_targets: $configuredTargets
failed_targets: $failedTargets
"@ | Set-Content -Path $engramStatus
                Write-Success "Applied Engram base setup"
            } else {
                @'
engram: missing
auto_configured: no
note: install the engram CLI, then rerun type extensions
'@ | Set-Content -Path $engramStatus
            ...
            $localSource = Join-Path $repoRoot "extensions\hooks\opencode-command-hooks"
            $installedSource = Join-Path $GA_DIR "extensions\hooks\opencode-command-hooks"
            $hooksSource = ""
            ...
            if (Test-Path $localSource) {
                ...
                Write-Error "Hooks plugin source not found."
                Write-Info "Checked: $localSource"
                Write-Info "Checked: $installedSource"
                return $false
            }

            Write-Info "Using hooks source: $hooksSource"

            # Check for bun (required by OpenCode for bundling)
            if (-not (Get-Command bun -ErrorAction SilentlyContinue)) {
                Write-Error "Bun is required to build hooks plugin (OpenCode uses Bun internally)"
                Write-Info "Install Bun: powershell -c 'irm bun.sh/install.ps1 | iex'"
                return $false
            }
            ...
            if (Test-Path $oldHooksDir) {
                Write-Info "Removing legacy plugin directory..."
            ...
            if ((Test-Path $opencodeJson) -and ((Get-Content $opencodeJson -Raw) -match "opencode-command-hooks")) {
                Write-Info "Removing legacy file: plugin reference from opencode.json..."
                & node -e "
                    const fs = require('fs');
            ...
            if ((Test-Path $ocPkg) -and ((Get-Content $ocPkg -Raw) -match "opencode-command-hooks")) {
                ...
                Write-Info "Cleared stale OpenCode package cache"
            }
...
'@
                ...
                Write-Info "command-hooks.jsonc already exists"
            }
            ...
            if (Test-Path $agentSource) {
                if (-not (Test-Path $agentTarget)) {
                    ...
                    Write-Info "Engineer agent already exists"
                }
            } else {
...
function Update-AllComponents {
    ...
    if ($gaStatus -eq "OUTDATED") {
        ...
        Write-Info "GA is up to date ($gaCurrent)"
    }
    ...
    if ($sddStatus -eq "NOT_INSTALLED" -or $sddStatus -eq "PARTIAL") {
        ...
        Write-Info "Refreshing SDD Orchestrator skills ($sddCurrent detected)..."
        if (Install-SDD -Action "update" -TargetDir $TargetDir) {
            $updated++
        } else {
            $failed++
    ...
    if ($failed -gt 0) {
        Write-Warning "Failed to update $failed component(s)"
        return $false
    }
...
function Apply-ProjectType {
    ...
    if (-not (Test-Path $typeDir)) {
        Write-Warning "Unknown project type '$ProjectType'. Falling back to 'nest'."
        $ProjectType = "nest"
        $typeDir = Join-Path $typesDir "nest"
        if (-not (Test-Path $typeDir)) {
            Write-Warning "nest type directory not found, skipping type application"
            return
        }
    }

    Write-Info "Applying project type: $ProjectType"
    ...
    if (Test-Path $agentsSource) {
        if (-not (Test-Path $agentsTarget) -or $Force) {
            ...
            $orchestratorTpl = Join-Path $autoDir "lib\templates\sdd-orchestrator.md"
            if (Test-Path $orchestratorTpl) {
                Add-Content -Path $agentsTarget -Value ""
                Add-Content -Path $agentsTarget -Value (Get-Content $orchestratorTpl -Raw)
            }
            
            # Append Engram template
            $engramTpl = Join-Path $autoDir "lib\templates\engram-protocol.md"
            if (Test-Path $engramTpl) {
                Add-Content -Path $agentsTarget -Value ""
                Add-Content -Path $agentsTarget -Value (Get-Content $engramTpl -Raw)
            }

            Write-Success "Copied and built AGENTS.md ($ProjectType)"
        } else {
            Write-Warning "AGENTS.md already exists, skipping (use -Force to overwrite)"
        }
    }
    ...
    if (Test-Path $reviewSource) {
        if (-not (Test-Path $reviewTarget) -or $Force) {
            ...
            Write-Warning "REVIEW.md already exists, skipping (use -Force to overwrite)"
        }
    }
    ...
    if ((Test-Path $typesJson) -and (Test-Path $mainSkillsDir)) {
        try {
            ...
            Write-Warning "Could not parse types.json: $_"
        }
        ...
        if ($skillsSourceNorm -ne $skillsTargetNorm) {
            if (-not (Test-Path $skillsTarget)) {
                ...
                Write-Warning "skills/ directory already exists in target, skipping copy"
            }
        }
    } else {
        Write-Warning "skills/ directory not found"
    }
...
if ($MyInvocation.InvocationName -ne '.') {
    ...
    switch ($command) {
        ...
        default {
            Write-Host "Usage: .\installer.ps1 {install-ga|install-sdd|install-vscode|configure|update-all}"
            exit 1
        }
    }
}
